...
def db_manager(temp_dir):
    """Create database manager with product factory schema"""
    db_path = temp_dir / "test.db"
    manager = DatabaseManager(db_path)
    
    # Initialize product factory tables using the actual schema
    from product_factory.core.factory import ProductFactory
    from core.engine import ConfigManager
    import yaml
    ...
    factory = ProductFactory(manager, config)
...
class TestIntegration:
    ...
    def test_full_lifecycle(self, output_dir, db_manager, config):
        """Test full product lifecycle"""
        factory = ProductFactory(db_manager, config)
